...
"""
    ...
    The first argument to bind is the src name *without* the subdomain.
...
class Map(object):
    ...
    def bind_to_environ(self, environ, server_name=None, subdomain=None):
        ...
        If `subdomain` is `None` but an environment and a src name is
        ...
        .. versionchanged:: 1.0.0
            If the passed src name specifies port 443, it will match
            if the incoming scheme is ``https`` without a port.

        .. versionchanged:: 1.0.0
            A warning is shown when the passed src name does not
            match the incoming WSGI src name.

        .. versionchanged:: 0.8
           This will no longer raise a ValueError when an unexpected src
           name was passed.
        ...
        :param server_name: an optional src name hint (see above).
        ...
        if subdomain is None and not self.host_matching:
            ...
            if cur_server_name[offset:] != real_server_name:
                # This can happen even with valid configs if the src was
                # accessed directly by IP address under some situations.
                # Instead of raising an exception like in Werkzeug 0.7 or
                # earlier we go by an invalid subdomain which will result
                # in a 404 error on matching.
                warnings.warn(
                    "Current src name '{}' doesn't match configured"
                    " src name '{}'".format(wsgi_server_name, server_name),
                    stacklevel=2,
                )
                subdomain = "<invalid>"
            else:
...
class MapAdapter(object):
    ...
    def build(
        ...
        external URLs (include the src name) will only be used if the
