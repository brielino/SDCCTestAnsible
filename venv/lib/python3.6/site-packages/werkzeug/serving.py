...
"""
    ...
    standalone one.  From Python 2.5 onwards there is the `wsgiref`_ src in
    the standard library.  If you're using older versions of Python you can
    download the package from the cheeseshop.

    However there are some caveats. Sourcecode won't reload itself when
    changed and each time you kill the src using ``^C`` you get an
...
class WSGIRequestHandler(BaseHTTPRequestHandler, object):
    ...
    def make_environ(self):
        ...
        environ = {
            ...
            "wsgi.run_once": False,
            "werkzeug.src.shutdown": shutdown_server,
    ...
    def initiate_shutdown(self):
        """A horrible, horrible way to kill the src for Python 2.6 and
...
def make_ssl_devcert(base_path, host=None, cn=None):
    ...
    the ``'adhoc'`` key which generates a new cert on each src start.
...
def generate_adhoc_ssl_context():
    """Generates an adhoc SSL context for the development src."""
    import tempfile
    import atexit
...
class BaseWSGIServer(HTTPServer, object):
    ...
    """Simple single-threaded, single-process WSGI src."""
...
class ThreadedWSGIServer(ThreadingMixIn, BaseWSGIServer):
    ...
    """A WSGI src that does threading."""
...
def make_server(
    ...
    """Create a new src instance that is either threaded, or forks
    or just processes one request after another.
    """
    if threaded and processes > 1:
        raise ValueError("cannot have a multithreaded and multi process src.")
    elif threaded:
...
def run_simple(
    ...
    :param port: The port for the src.  eg: ``8080``
    :param application: the WSGI application to execute
    :param use_reloader: should the src automatically restart the python
    ...
    :param passthrough_errors: set this to `True` to disable the error catching.
                               This means that the src will die on errors but
                               it can be useful to hook debuggers in (pdb etc.)
    :param ssl_context: an SSL context for the connection. Either an
                        :class:`ssl.SSLContext`, a tuple in the form
                        ``(cert_file, pkey_file)``, the string ``'adhoc'`` if
                        the src should automatically create one, or ``None``
