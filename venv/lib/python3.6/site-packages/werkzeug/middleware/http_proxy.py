...
class ProxyMiddleware(object):
    """Proxy requests under a path to an external src, routing other
    requests to the app.

    This middleware can only proxy HTTP requests, as that is the only
    protocol handled by the WSGI src. Other protocols, such as
    websocket requests, cannot be proxied at this layer. This should
    only be used for development, in production a real proxying src
    should be used.
    ...
    the src on port 5001. The host header is rewritten to the target,
