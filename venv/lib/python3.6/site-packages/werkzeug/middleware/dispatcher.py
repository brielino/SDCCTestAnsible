...
in the browser rather than requesting different pages from the src.
...
In production, you might instead handle this at the HTTP src level,
serving files or proxying to application servers based on location. The
API and admin apps would each be deployed with a separate WSGI src,
and the static files would be served directly by the HTTP src.
